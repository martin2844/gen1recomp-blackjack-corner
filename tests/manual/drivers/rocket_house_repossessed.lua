return function(game)
  local U = dofile("tests/drivers/util.lua")
  U.wait(10)
  local loader = assert(game.mods, "mod loader is unavailable")
  local exports = assert(loader.exports and loader.exports.blackjack_corner,
    "blackjack_corner is not loaded")
  local bucket = loader.modSave.blackjack_corner or {}
  loader.modSave.blackjack_corner = bucket
  bucket.gamble_mode = true
  bucket.gamble_campaign = exports.campaign_state.defaults()
  local home = bucket.gamble_campaign.house
  home.status, home.bailoutClaimed = "ROCKET_OWNED", true
  game.save.inventory = game.save.inventory or {}
  game.save.inventory.COIN_CASE = 1
  exports.house_world.sync(game, exports.house)

  local shotDir = os.getenv("SHOT_DIR") or "/tmp/blackjack-corner-v06/house"
  U.teleport(game, "REDS_HOUSE_1F", 3, 6, "up")
  U.wait(180)
  U.shot(game, shotDir .. "/rocket-occupied-downstairs.png")
  U.teleport(game, "REDS_HOUSE_2F", 3, 4, "left")
  U.wait(180)
  U.shot(game, shotDir .. "/mom-upstairs.png")
  U.tap(game, "a")
  U.wait(180)
  U.shot(game, shotDir .. "/mom-upstairs-dialogue.png")
  U.log("Rocket-owned family home prepared; Mom is immediately to the left.")
  U.log("Verify Mom heals, then walk downstairs and test stairs, exits, and dialogue.")
  U.log("MANUAL CONTROL READY")
  while true do coroutine.yield() end
end
